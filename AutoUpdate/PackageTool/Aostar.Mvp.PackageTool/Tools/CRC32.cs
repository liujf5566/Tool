...
namespace AppUpdate.Communal
{
    ...
    public class CRC32
    {
        ...
        /// <returns></returns>
        public uint ByteCRC(byte[] _buffer)
        {
            uint ulCRC = 0xffffffff;
            uint len;
            len = (uint)_buffer.Length;
            for (uint buffptr = 0; buffptr < len; buffptr++)
            {
                uint tabPtr = ulCRC & 0xFF;
                tabPtr = tabPtr ^ _buffer[buffptr];
                ulCRC = ulCRC >> 8;
        ...
        /// <summary>
        /// 计算CRC 第一执行_ulCRC 传:0xffffffff
        /// </summary>
        /// <param name="_buffer">字节数组</param>
        /// <param name="ulCRC">out 保留状态</param>
        /// <returns></returns>
        public uint ByteCRC(byte[] _buffer, ref uint _ulCRC)
        {
            uint ulCRC;
            uint len;
