...
namespace ExportData
{
    public partial class DataForm : Form
    {
        ...
        {
            DataTable dt = DataHelper.CreateDataTableByExcel(txtFile.Text, 0);
        ...
        private void btnCreate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //开始生成内存数据
                DataTable dt = DataHelper.CreateDataTableByExcel(txtFile.Text, 1);
                List<PersonalPerformance> pps = DataHelper.CreatePersonalPerformance(dt, "研发中心", 3);
                var noTaskPPS = DataHelper.CreatePersonalPerformance(_persons, pps, "研发中心", 3);
                pps.AddRange(noTaskPPS);
                List<int> scores = DataHelper.GetSortedScores(pps);
                //开始生成新的Excel
                string path = "";
